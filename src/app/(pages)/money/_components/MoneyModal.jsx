...
const MoneyModal = ({ modalPopup, setModalPopup, itemInfo }) => {
  const itemsId = itemInfo?.id;

  const router = useRouter();
  const deleteMoney = async () => {
    const userCollection = doc(db, "money", itemsId);
  ...
  return (
    <div className={modelClass}>
      <div className={modelOverlay} onClick={() => setModalPopup(false)} />
      <div className={modelMain}>
        ...
        <div className="flex flex-col bg-[#231f32] rounded-xl text-white text-lg font-medium items-start mt-5">
          <Link
            href={`/money/eidtMoney?itemId=${itemInfo?.id}`}
